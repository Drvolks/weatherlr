...
public class RssEntryToWeatherInformation {
    ...
    public func performRegex(_ regex: NSRegularExpression, text: String, index: Int) -> String {
        ...
        if let result = results.first, let range = Range(result.range(at: index), in: text) {
            let condition = String(text[range]).trimmingCharacters(in: .whitespacesAndNewlines)
            return condition
        }
    ...
    public func nettoyerDetail(_ text: String) -> String {
        ...
        let result = regex.stringByReplacingMatches(in: text, options: .withTransparentBounds, range: NSRange(text.startIndex..., in: text), withTemplate: "")
